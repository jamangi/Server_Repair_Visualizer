...
export const cableNodes: CableNode[] = [
  ...
  { id: 'busbar', name: '12 V inner busbar', shortLabel: '12 V BUS', x: 455, y: 245, width: 90, height: 215, form: 'slim', zone: 'rear', note: 'Paired rigid power rails and their partition occupy the centerline between the Bianca boards. The red-and-black busbar power cable is shown as the route to the PSB.' },
  { id: 'fan-bank', name: 'Fan modules 1–8', shortLabel: 'FANS 1–8', x: 85, y: 510, width: 830, height: 92, form: 'assembly', zone: 'fan-wall', note: 'Eight modules span the chassis immediately in front of the Bianca boards. Dell explicitly names Fan 1 and Fan 2 connections to the left Bianca.' },
  { id: 'bmc', name: 'Baseboard Management Controller board', shortLabel: 'BMC', x: 120, y: 670, width: 145, height: 145, form: 'medium', zone: 'middle', note: 'Compact control board in the tray immediately in front of the fan wall, beside the PDB.' },
  { id: 'pdb', name: 'Power Distribution Board', shortLabel: 'PDB', x: 300, y: 660, width: 230, height: 170, form: 'medium', zone: 'middle', note: 'Compact power board and tray immediately in front of the fan wall, enlarged enough to expose its J4 study endpoint.' },
  { id: 'interposer', name: 'Interposer board', shortLabel: 'INT', x: 565, y: 675, width: 90, height: 120, form: 'slim', zone: 'middle', note: 'Skinny connector board in the PDB/BMC zone. HMC and BMC endpoints are retained as lesson observations.' },
...
export const cableRoutes: CableRoute[] = [
  ...
  { id: 'hmc-interposer', from: 'hmc', to: 'interposer', label: 'HMC cable', kind: 'management', evidence: 'lesson-observation', path: 'M92 305 V470 H495 V640 H610 V675', routeNote: 'The lesson maps an HMC cable to the interposer; the route uses a reserved gap through the fan wall, while public endpoint evidence remains incomplete.', sourceId: 'lesson-walkthrough' },
  { id: 'bmc-interposer', from: 'bmc', to: 'interposer', label: 'BMC cable', kind: 'management', evidence: 'lesson-observation', path: 'M192 670 V640 H690 C690 620 710 620 710 640 H720 V675 H655', routeNote: 'The raised arc at x=700 shows this cable jumping over the BlueField-3 data lane.', sourceId: 'lesson-walkthrough', bridge: { x: 700, y: 640 } },
  ...
  { id: 'bf3-bianca', from: 'bf3', to: 'bianca-left', label: 'BF3 / left Bianca cable set', kind: 'data', evidence: 'documented', path: 'M340 980 H700 V430 H650', routeNote: 'Uses a reserved gap through the fan wall and stays clear of the PDB/BMC/interposer tray.', sourceId: 'dell-left-bay' },
  { id: 'ultrapass-bianca', from: 'left-riser', to: 'bianca-left', label: 'Ultra-Pass ×2', kind: 'data', evidence: 'endpoint-review', path: 'M389 930 V850 H288 V455 H335', routeNote: 'Dell specifies two connectors at the left Bianca and the fan-channel route; the exact left-bay card endpoint still needs confirmation.', sourceId: 'lesson-walkthrough' },
  { id: 'fan12-bianca', from: 'fan-bank', to: 'bianca-left', label: 'Fan 1 / Fan 2 power', kind: 'power', evidence: 'documented', path: 'M205 510 V435', routeNote: 'Dell says these two fan cables route over the cold-plate tubing and connect to the left Bianca.', sourceId: 'dell-left-bay' },
  { id: 'busbar-psb', from: 'busbar', to: 'psb', label: 'Red / black busbar power cable', kind: 'power', evidence: 'documented', path: 'M500 460 H803 V915', routeNote: 'Leaves the central busbar area, passes through a reserved fan-wall gap, and stays flat in the black plastic tray before connecting to the PSB.', sourceId: 'dell-left-bay' },
  { id: 'psb-bianca', from: 'psb', to: 'bianca-right', label: 'PSB PCIe path', kind: 'data', evidence: 'endpoint-review', path: 'M900 915 H920 V620 H780 V430 H650', routeNote: 'The PSB/riser cable is documented, but the complete board-to-board endpoint needs procedure-level confirmation.', sourceId: 'lesson-walkthrough' },
  { id: 'drives-backplane', from: 'drives', to: 'e1s-backplane', label: 'E1.S edge connectors ×8', kind: 'direct', evidence: 'lesson-observation', path: 'M505 1330 V1155', routeNote: 'Each carrier seats directly into a backplane connector; the split into lesson-numbered sections remains under review.', sourceId: 'lesson-walkthrough' },
